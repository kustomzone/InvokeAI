...
class Installer:
    ...
    def install(self, root: str = "~/invokeai", version: str = "latest", yes_to_all=False) -> None:
        ...
        if FF_USE_WHEEL:
            self.instance.deploy_wheel(extra_index_url=get_torch_source() if not yes_to_all else None)
        else:
            self.instance.deploy_src(extra_index_url=get_torch_source() if not yes_to_all else None)
